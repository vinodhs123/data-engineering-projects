...
{{ latest_folder_ibp(
    source_path="abfss://" ~ container_value2 ~ "@" ~ storage_account_value ~ ".dfs.core.windows.net/ibp/AZR_ADJPRODRESHIST",
    files_format = "csv",
    delimiter = ',',
	...
	all_columns = [
		...
		"CASE WHEN ADJPRODRESHIST = '<Null>' THEN 0 ELSE CAST(CAST(ADJPRODRESHIST AS DECIMAL(18,6)) AS INT) END AS PLANNED_PRODUCTION"
	]

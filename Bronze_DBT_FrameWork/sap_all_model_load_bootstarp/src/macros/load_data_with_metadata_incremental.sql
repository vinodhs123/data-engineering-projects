{% macro load_data_with_metadata_incremental(
    historical_source,
    incremental_source,
    all_columns,
    composite_keys,
...
{% set checkpoint_tbl = var('ops_catalog') ~ ".edp_ops.checkpoint" %}
...
WITH incremental_raw AS (
    ...
    (select file_name from {{ checkpoint_tbl }}
    where table_name= '{{current_model_name}}')
)
